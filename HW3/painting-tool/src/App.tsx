...
function App() {
  ...
  const downloadCanvas = () => {
    if (!username) return;
    console.log(`Downloading canvas for user: ${username}`);
    fetch(`http://localhost:8080/api/paint/${username}`)
      .then(res => res.json())
  ...
  const uploadCanvas = () => {
    console.log(`Uploading canvas for user: ${username}`);
    fetch(`http://localhost:8080/api/paint/${username}`, {
      method: 'POST',
      ...
      <div className='top-bar'>
        <div>
          ...
          <input
            ...
            style={{ fontSize: '16px', fontWeight: 'bold', border: 'none', background: 'transparent', width: '100px' }}
          />
          <button style={{ marginLeft: '16px' }} onClick={uploadCanvas}>Upload</button>
          <button style={{ marginLeft: '16px' }} onClick={downloadCanvas}>Download</button>
        </div>
        <input
          type='text'
          value={title}
